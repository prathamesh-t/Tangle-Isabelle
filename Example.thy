  ...
  moreover have "codomain_wall  ((basic(cup#[]))\<circ>(basic(cup#vert#vert#[])))
   ...
   ultimately have "((basic [cup,cup]) \<circ> (?temp)) 
    ...
    have "((basic [cup,cup]) \<circ> (?temp)) 
                 ~ (basic [cup] \<circ> (left_over \<otimes> straight_line))"
              using 8 55 compose_leftassociativity sym wall.simps 
              Tangle_Equivalence.sym compose_Nil  
              by (metis)    
    moreover have "(basic [cup]) \<circ> (left_over \<otimes> straight_line) 
                    ~ (basic [cup]) \<circ> (straight_line \<otimes> straight_line)"
                  using 4 by auto
...
\<otimes>((basic (vert#vert#[]))\<circ>(basic (cap#[])))"
 ...
 moreover  have 15: "((?z) \<circ> ((basic(cap#cap#[]))) \<circ> (basic ([]))) 
               ...
               by (metis (hide_lams, mono_tags) Tangle_Equivalence.compose_eq Tangle_Equivalence.refl `codomain_wall (basic [cup, cup]) = domain_wall (basic [vert, over, vert] \<circ> basic [cap, vert, vert])` `domain_wall (basic [cap, cap] \<circ> basic []) = codomain_wall (basic [cup, cup] \<circ> basic [vert, over, vert])` comp_of_tangle_dgms domain_wall_compose is_tangle_diagram.simps(1))
