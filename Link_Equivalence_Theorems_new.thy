theory Link_Equivalence_Theorems_new
imports Tangles
begin

(*-tasks
1) Commenting
2) Modification of further lemmas
3) example.thy*)
...
"linkrel_equiv (Abs_diagram (x1 \<circ> basic y1 \<circ> basic ?z4\<circ>basic ?z4\<circ>z1)) 
               ...
               using compose_leftassociativity linkrel_doublecompress_top assms 
               non_seperating_block_def by auto
(*step 2 - inducing is_cup on the right*)
...
proof-
 ...
 ultimately have 5: " (snd (wall_count x1)) = (fst (count y1))" 
   by auto
...
(*these are the helper lemmas that are used in simplification and clarification later*)
 ...
 "linkrel_equiv 
           (Abs_diagram (x1 \<circ> basic ?z4\<circ>basic ?z4 \<circ> basic y1\<circ>z1)) 
           (Abs_diagram (x1 \<circ> (basic y1)\<circ>z1))" 
         using compose_leftassociativity rtranclp_trans linkrel_doublecompress_below assms(1) 
         by auto
...
theorem metaequivalence_left_drop: 
 ...
 proof- 
 have "fst (brickcount cup) = 0" using brickcount_def by auto
 then have subresult2:"fst (count ((cement cup))) = 0" using count_def  count.simps(1) 
       by (metis)
...
proof-
 have "fst (brickcount cup) = 0" using brickcount_def by auto
 then have subresult2:"fst (count ((cement cup))) = 0" 
...
proof-
 ...
 then have "snd (count (y2 \<otimes> (cement vert) \<otimes> (cement vert))) > (snd (count y2))" 
   by auto
 then have step1: "snd (count (?p2)) > 1" 
   using assms  by auto
 ...
 from subresult2 have "snd (count ?p2) = (snd (count y2)) + 2"  
   by auto
 then have subresult5: "w5 = make_vert_block (nat (((snd (count ?p2)) + (-2))))" 
    using assms by auto
...
proof-
 ...
 have subresult2:"(snd (count (?y2))) = (snd (count y1)) + 2"
      using  count_def snd_conv append_blocks_Cons count_cup_rightcompose by auto
 with assms have subresult3:"(snd (count (?y2))) >1" by auto
 from subresult2 have subresult4: "w5 = make_vert_block (nat (snd (count ?y2) + (-2)))" 
      using assms by auto
 with assms and subresult3 and metaequivalence_left have 
  "linkrel_equiv 
...
proof-
 ...
 have "snd (count (w4\<otimes>(cement cup)\<otimes>(cement vert))) = snd (count w4) + snd (count (cement cup)) +
     ...
     by (auto)
 then have subresult5: "snd (count (w4\<otimes>(cement cup)\<otimes>(cement vert))) = snd (count w4) +3"
 ...
 then have subresult6: "fst (count (w4\<otimes>(cement cup)\<otimes>(cement vert))) =  fst (count w4) + 1"
 ...
 with subresult12 have "snd(wall_count x1) = (fst (count y1))" by simp
 with subresult10 have subresult13: 
 ...
 from subresult14 and this have subresult15: "fst(wall_count z1) =  snd (count y1)" 
       by simp
 with subresult4 have subresult16: "fst(wall_count z1) = snd (count ?y2)" 
       by simp
 have "(fst (wall_count ((basic (w4\<otimes>(cement cup)\<otimes>(cement vert)))\<circ>(basic ?y2)\<circ>z1))) 
       ...
       by auto
 with  subresult13 have subresult17: 
        "snd(wall_count x1) 
 ...
 have  "list_sum (wall_count_list (?y2*z1)) = 0" 
        ...
        by (metis)
 then have subresult18: " list_sum (wall_count_list ((basic ?y2)\<circ>z1)) = 0" 
        by auto
 ...
 with subresult19 and subresult20 and subresult17 
     have subresult21: 
      "list_sum (wall_count_list ((x1)\<circ>(basic (w4\<otimes>(cement cup)\<otimes>(cement vert)))
                                  \<circ>(basic (y1\<otimes>(cement cap)))\<circ>z1)) 
                         = 0"
 ...
 have subresult26: 
  "(fst (wall_count (((x1)\<circ>(basic (w4\<otimes>(cement cup)\<otimes>(cement vert)))\<circ>(basic (y1\<otimes>(cement cap)))\<circ>z1))))
                         = fst(wall_count x1)" 
          using wall_count_def compose_Cons by (metis fst_conv wall_count_compose)
 with subresult23 have subresult27:
 ...
 with assms and subresult3 and metaequivalence_bottomleft and subresult30
  have "linkrel_equiv 
...
proof-
 ...
 with assms have subresult3:"fst (count ?w2) = snd (count y1)" 
        by auto
 ...
 "linkrel_equiv
        (Abs_diagram ((x1)\<circ>(basic ((cement cup)\<otimes>y1))
         ...
         (Abs_diagram (x1\<circ>(basic y1)\<circ>(basic ((cement cup)\<otimes>y2\<otimes>(cement cup)))
                      ...
                      \<circ>(basic (w4\<otimes>(cement cap)\<otimes>(cement vert)))\<circ>z1))"
     using linkrel_equiv_def r_into_rtranclp by metis
 with subresult1 and r_into_rtranclp rtranclp_trans have subresult5:
     "linkrel_equiv
             (Abs_diagram ((x1)\<circ>(basic ((cement cup)\<otimes>y1))
                \<circ>(basic ((cement vert)\<otimes>(cement vert)\<otimes>y2\<otimes>(cement cup)))
                \<circ>(basic ((cement vert)\<otimes>(cement vert)\<otimes>y3\<otimes>(cement vert)\<otimes>(cement vert)))
 ...
 with assms have subresult6: "snd (count ((cement cup) \<otimes> y1)) = 2 + fst (count y2)" 
       by auto
 ...
 with subresult6 have "snd (count (?subs1)) = fst (count (?subs2))" 
       by auto
 with subresult2 and subresult4 have
 ...
 "linkrel_equiv
    ...
    (Abs_diagram ((x1)\<circ>(basic ((cement cup)\<otimes>y1))
            ...
            \<circ>(basic ((cement vert)\<otimes>(cement cap)\<otimes> w5))\<circ>(basic (w4\<otimes>(cement cap)\<otimes>(cement vert)))\<circ>z1))" 
       by metis
 with subresult5 and r_into_rtranclp rtranclp_trans have
  "linkrel_equiv
      (Abs_diagram ((x1)\<circ>(basic ((cement cup)\<otimes>y1 \<otimes> (cement cup)))
                  \<circ>(basic ((cement vert)\<otimes>(cement vert) \<otimes> y2 \<otimes> (cement vert) \<otimes> (cement vert)))
                  \<circ>(basic ((cement vert)\<otimes>(cement vert)\<otimes>y3\<otimes>(cement vert)\<otimes>(cement vert)))
