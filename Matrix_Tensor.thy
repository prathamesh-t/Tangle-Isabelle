<<<<<<< HEAD
header{*Tensor product of matrices*}
=======
>>>>>>> 7fd971d3a18e8b15f3ef550f3c1fd8e0c2e454ad
...
=======
lemma row_length_vect_mat: " row_length (vec_mat_Tensor v m) = length v*(row_length m)"
proof(induct m)
 case Nil
  have "row_length [] = 0" using row_length_Nil by simp
  moreover have "vec_mat_Tensor v [] = []" using vec_mat_Tensor.simps(1) by auto 
  ultimately have " row_length (vec_mat_Tensor v []) = length v*(row_length [])" using mult_0_right by (metis )
  from this show ?case by metis
 next  
  fix a m
  assume A:"row_length (vec_mat_Tensor v m) = length v * row_length m"
  let ?case = "row_length (vec_mat_Tensor v (a#m)) = (length v)*(row_length (a#m))" 
  have A:"row_length (a # m) = length a" using row_length_def by (metis hd.simps list.distinct(1))
  have "(vec_mat_Tensor v  (a#m)) = (vec_vec_Tensor v a)#(vec_mat_Tensor v m)" using vec_mat_Tensor_def vec_mat_Tensor.simps(2)
  by auto
  from this have "row_length (vec_mat_Tensor v (a#m)) = length (vec_vec_Tensor v a)" using row_length_def  hd.simps
  by (metis list.distinct(1) vec_mat_Tensor.simps(2))
  from this and vec_vec_Tensor_length have "row_length (vec_mat_Tensor v (a#m)) = (length v)*(length a)" by auto
  from this and A  have "row_length (vec_mat_Tensor v (a#m)) = (length v)*(row_length (a#m))"
  by auto
  from this show ?case by auto
qed
...
proof(induct m1)
 ...
 have "row_length ((a # m1) \<otimes> m2) = row_length (a # m1) * row_length m2"
   ...
   next
    ...
    from this have "(vec_mat_Tensor a (aa#m2)) \<noteq> []" using vec_mat_Tensor_def by auto
    from this have "hd ((vec_mat_Tensor a (aa#m2))@(m1\<otimes>m2))= hd (vec_mat_Tensor a (aa#m2)) " by auto

    from this have "hd ((a#m1)\<otimes>(aa#m2)) = hd (vec_mat_Tensor a (aa#m2))" using Tensor.simps(2) by auto
    from this have s1: "row_length ((a#m1)\<otimes>(aa#m2)) = row_length (vec_mat_Tensor a (aa#m2))" 
           using row_length_def by (metis Nil_is_append_conv `vec_mat_Tensor a (aa # m2) \<noteq> []` Tensor.simps(2))
    have "row_length (vec_mat_Tensor a (aa#m2)) = (length a)*row_length(aa#m2)" using row_length_vect_mat
    by metis   
    from this and s1  
     have "row_length (vec_mat_Tensor a (aa#m2)) = (length a)*row_length(aa#m2)"
         by auto
   from this and B have "row_length (vec_mat_Tensor a (aa#m2)) = (row_length (a#m1))*row_length(aa#m2)"    
        by auto
   from this  and s1 show ?case  by (auto)
   qed
  from this show ?case by auto
...
proof(induct M) 
 case Nil
  have "(vec_mat_Tensor v []) = []" using vec_mat_Tensor.simps(1) Nil  by simp
  moreover have "(row_length [] = 0)"  using row_length_def Nil by metis
  moreover have "(length []) = 0" using Nil by simp
  ultimately have "mat ((row_length [])*(length v)) (length []) (vec_mat_Tensor v [])" using zero_matrix by (metis mult_zero_left)
  from this show ?case by simp
 next
 fix a M
 assume hyp :"(mat (row_length M) (length M) M \<Longrightarrow> mat (row_length M * length v) (length M) (vec_mat_Tensor v M))"
               " mat (row_length (a#M)) (length (a#M)) (a#M)"                      
  let ?case = "mat (row_length (a#M) * length v) (length (a#M)) (vec_mat_Tensor v (a#M))"
  have step1: "mat (row_length M) (length M) M" using hyp(2) reduct_matrix by auto
  from this have step2:"mat (row_length M * length v) (length M) (vec_mat_Tensor v M)" using hyp(1) by auto 
  have "mat (row_length (a#M) * length v) (length (a#M)) (vec_mat_Tensor v (a#M))" 
    proof (cases M)
         case Nil 
     have 1:"(vec_mat_Tensor v (a#M)) = [vec_vec_Tensor v a]" using vec_mat_Tensor.simps Nil by auto
     have   "(x \<in> (set [vec_vec_Tensor v a])) \<longrightarrow>  x = (vec_vec_Tensor v a)" using set_def by auto
     from this have 2:" (x \<in> (set [vec_vec_Tensor v a])) \<longrightarrow> (vec (length (vec_vec_Tensor v a)) x)" using vec_def by metis 
     have "length (vec_vec_Tensor v a) = (length v)*(length a)" using vec_vec_Tensor_length by auto 
     from this have "length (vec_vec_Tensor v a) = (length v)*(row_length (a#M))" using row_length_def hd.simps
     list.distinct(1) by metis
     from this and 2 have "(x \<in> (set [vec_vec_Tensor v a])) \<longrightarrow> (vec ((length v)*(row_length (a#M))) x)" by auto
     from this and 1 have 3:"(x \<in> set (vec_mat_Tensor v (a#M))) \<longrightarrow> vec ((length v)*(row_length (a#M))) x" 
     by auto
     have 4: "length (vec_mat_Tensor v (a#M)) = (length (a#M))" using vec_mat_Tensor_length by auto
     from this have "mat (row_length (a#M) * length v) (length (vec_mat_Tensor v (a#M))) (vec_mat_Tensor v (a#M))"
     using mat_def Ball_def by (metis (hide_lams, no_types) 
    "1" `length (vec_vec_Tensor v a) = length v * row_length (a # M)` `length (vec_vec_Tensor v a) = length v * length a` 
    hd_set in_set_insert insert_Nil length_code nat_mult_commute not_Cons_self2 vec_vec_Tensor_length vec_def)
     ...
     case (Cons b L)
      ...
      have "  mat (row_length M * length v) (length M) (vec_mat_Tensor v M)" using step2 by auto 
      from this have " Ball (set (vec_mat_Tensor v M)) (vec ((row_length M)*(length v)))" using mat_def by auto
      from this have "(x \<in> set (vec_mat_Tensor v M)) \<longrightarrow> (vec ((row_length M)*(length v)) x)" using mat_def Ball_def
      by auto
      from this have 3:" (x \<in> set (vec_mat_Tensor v M)) \<longrightarrow> (vec ((length a)*(length v)) x)" using 2 by auto
  
      have "length (vec_vec_Tensor v a) = (length a)*(length v)" by (metis nat_mult_commute vec_vec_Tensor_length)  
      from this  have 4:" vec ((length a)*(length v)) (vec_vec_Tensor v a)" using vec_vec_Tensor_length vec_def 
             by (metis (full_types))

      have 5:"(length a) = (row_length (a#M))" using row_length_def hd.simps  list.distinct(1)  by (metis) 
 
      have "vec_mat_Tensor v (a#M) = (vec_vec_Tensor v a)#(vec_mat_Tensor v M)" using vec_mat_Tensor.simps(2) by auto
      from this have "(x \<in> set (vec_mat_Tensor v (a#M)))\<longrightarrow> ((x = (vec_vec_Tensor v a)) \<or> (x \<in> (set (vec_mat_Tensor v M)))) "
      using hd.simps hd_set by auto
      from this 3 4 have "(x \<in> set (vec_mat_Tensor v (a#M)))\<longrightarrow>  vec ((length a)*(length v)) x" by auto
      from this 5 have "(x \<in> set (vec_mat_Tensor v (a#M)))\<longrightarrow>  vec ((row_length (a#M))*(length v)) x" by auto
      from this have "\<forall>x.((x \<in> set (vec_mat_Tensor v (a#M)))\<longrightarrow>  vec ((row_length (a#M))*(length v)) x)"
       by (metis "2" "4" "5" `Ball (set (vec_mat_Tensor v M)) (vec (row_length M * length v))` 
      hd_set vec_mat_Tensor.simps(2))
      from this have 6: "Ball (set (vec_mat_Tensor v (a#M))) (vec ((row_length (a#M))*(length v)))" using Ball_def by 
      auto
      have 7:"length (vec_mat_Tensor v (a#M)) = length (a#M)" using vec_mat_Tensor_length by auto
    
      from 6 and 7 have "mat ((row_length (a#M))*(length v)) (length (a#M)) (vec_mat_Tensor v (a#M))"
        using mat_def
        by (metis (hide_lams, no_types) "5" `length (vec_vec_Tensor v a) = length a * length v` length_code)
     from this show ?thesis by auto
     qed
    from hyp this show ?case by auto  
qed
...
=======
 have "((a # M1) \<otimes> M2) = (vec_mat_Tensor a M2)@(M1 \<otimes> M2)" using Tensor.simps(2) by auto
 from this have 1:"length ((a # M1) \<otimes> M2) = length ((vec_mat_Tensor a M2)@(M1 \<otimes> M2))" by auto
 have 2:"length ((vec_mat_Tensor a M2)@(M1 \<otimes> M2)) = length (vec_mat_Tensor a M2)+ length (M1 \<otimes> M2)" using append_def
 by auto
 have 3:"(length (vec_mat_Tensor a M2)) = length M2" using vec_mat_Tensor_length by (auto)
 have 4:"length (M1 \<otimes> M2) = (length M1)*(length M2)" using  Cons.hyps by auto
 from this 2 3 have  "length ((vec_mat_Tensor a M2)@(M1 \<otimes> M2)) =  (length M2) + (length M1)*(length M2)"
 by auto
 from this have 5:"length ((vec_mat_Tensor a M2)@(M1 \<otimes> M2)) =  (1 + (length M1))*(length M2)" by auto
...
=======
 ...
 from this show?thesis by simp
qed
(*proves that Tensor vec_vec_Tensor takes well defined matrices to well defined matrices*)
theorem well_defined_Tensor:
...
proof(induct M1)
 ...
 have "mat (row_length (a # M1) * row_length M2) (length (a # M1) * length M2) (a # M1 \<otimes> M2)"
  proof(cases M1)
   case Nil 
    have "(mat ((row_length M2)*(length a)) (length M2) (vec_mat_Tensor a M2))" using Cons.prems 
    well_defined_vec_mat_Tensor by auto
    moreover have "(length (a # M1)) * (length M2) = length M2" using Nil by auto
    moreover have "(a#M1)\<otimes>M2 = (vec_mat_Tensor a M2)" using Nil Tensor.simps append.simps(1) by auto
  ...
  case (Cons b N1)
     ...
     from this have "x\<in> (set (a#M1)) \<longrightarrow> (vec (row_length (a#M1)) x)" using mat_def Ball_def by metis
      ...
      have "(mat ((row_length M2)*(length a)) (length M2) (vec_mat_Tensor a M2))" using well_defined_vec_mat_Tensor
       Cons.prems by auto
      from this have "Ball (set (vec_mat_Tensor a M2)) (vec ((row_length M2)*(length a)))" using mat_def
         by auto
      from this have 4:"\<forall>x. x \<in> (set (vec_mat_Tensor a M2)) \<longrightarrow> (vec ((length a)*(row_length M2)) x)"
           using  nat_mult_commute by metis
      from this and 3 have 5: "\<forall>x. (x \<in> (set (vec_mat_Tensor a M2)))\<or>(x \<in> (set (M1 \<otimes> M2))) 
                         \<longrightarrow> (vec ((length a)*(row_length M2)) x)"     by auto  

       have 6:"(a # M1 \<otimes> M2) = (vec_mat_Tensor a M2)@(M1 \<otimes>M2)" using Tensor.simps(2) by auto 
...
lemma vec_vec_Tensor_simpl: "i<(length y) \<longrightarrow> (vec_vec_Tensor (z#zs) y)!i = (times z y)!i" 
proof-
have a: "vec_vec_Tensor (z#zs) y = (times z y)@(vec_vec_Tensor zs y)" by auto
from this have b: "length (times z y) = (length y)" using preserving_length by auto
from this have "i<(length (times z y)) \<longrightarrow> ((times z y)@(vec_vec_Tensor zs y))!i = (times z y)!i" using append_simpl
by metis
from this b have "i<(length y) \<longrightarrow> ((times z y)@(vec_vec_Tensor zs y))!i = (times z y)!i" by auto
from this a have "i<(length y) \<longrightarrow> (vec_vec_Tensor (z#zs) y)!i = (times z y)!i" by auto
from this show ?thesis by auto
qed


lemma vec_vec_Tensor_simpl2: "(i \<ge> (length y)) \<longrightarrow> ((vec_vec_Tensor (z#zs) y)!i = (vec_vec_Tensor zs y)!(i- (length y)))" 
using vec_vec_Tensor.simps(2) append_simpl2  preserving_length by metis
...
=======
\<longrightarrow> ((vec_vec_Tensor x y)!i) = f (x!(i div (length y))) (y!(i mod (length y))))"
 apply(rule allI)
 proof(induct x)
 case Nil
 have "(length [] = 0)" by auto
 also have "length (vec_vec_Tensor [] y) = 0" using vec_vec_Tensor.simps(1) by auto
 from this have "i <(length (vec_vec_Tensor [] y)) \<Longrightarrow> False" by auto
 moreover have "(vec_vec_Tensor [] y) = []" by auto 
 moreover have "(i<(length (vec_vec_Tensor [] y))) \<longrightarrow> 
 ((vec_vec_Tensor x y)!i) = f (x!(i div (length y))) (y!(i mod (length y)))"  
 by auto
 from this show ?case  by auto
 next
 case (Cons z zs)
 have 1:"vec_vec_Tensor (z#zs) y = (times z y)@(vec_vec_Tensor zs y)" by auto
 have 2:"i<(length y)\<longrightarrow>((times z y)!i = f z (y!i))" using times_elements by auto
 moreover have 3:"i<(length y) \<longrightarrow> (vec_vec_Tensor (z#zs) y)!i = (times z y)!i" using vec_vec_Tensor_simpl by auto
 moreover  have "i<(length y) \<longrightarrow> (vec_vec_Tensor (z#zs) y)!i = f z (y!i)" by (metis calculation(1) calculation(2))
 ...
 from this 3 have step1:"((i < (length y)) \<longrightarrow> 
  ((i<((length x)*(length y)) \<longrightarrow> ((vec_vec_Tensor (z#zs) y)!i 
  =  f  ((z#zs)!(i div (length y))) (y! (i mod (length y)))))))"
 by auto
 have "((length y) \<le> i) \<longrightarrow> (i - (length y)) \<ge> 0" by auto
 have step2:" ((length y) < i) \<longrightarrow>
  ((i < (length (z#zs)*(length y)))\<longrightarrow>((vec_vec_Tensor (z#zs) y)!i) 
   = f ((z#zs)!(i div (length y))) (y!(i mod (length y))))"
  proof-
  ...
  by auto
  have 4:" (i > (length y))\<longrightarrow>(vec_vec_Tensor (z#zs) y)!i =  (vec_vec_Tensor zs y)!(i- (length y))" using vec_vec_Tensor_simpl2 
  by auto
  ...
  \<longrightarrow> ((vec_vec_Tensor zs y)!i) = f (zs!(i div (length y))) (y!(i mod (length y))))" using Cons.hyps by auto
  from this 5 have "(i > (length y))\<longrightarrow>((i<((length (z#zs))*(length y)))
  \<longrightarrow> ((vec_vec_Tensor zs y)!(i -(length y))) = f (zs!((i -(length y)) div (length y))) (y!((i -(length y)) 
   mod (length y))))
   = ((i<((length zs)*(length y)))
  \<longrightarrow> ((vec_vec_Tensor zs y)!i) = f (zs!(i div (length y))) (y!(i mod (length y))))
    " by auto
  ...
  \<longrightarrow> ((vec_vec_Tensor (z#zs) y)!i) = f ((z#zs)!(i div (length y))) (y!(i mod (length y))))"
  by auto
  from this show ?thesis  by auto
  qed
 ...
 ((i < (length (z#zs)*(length y)))\<longrightarrow>((vec_vec_Tensor (z#zs) y)!i) 
   = f ((z#zs)!(i div (length y))) (y!(i mod (length y))))"
  proof-
  have 1:"(i = (length y)) \<longrightarrow> ((vec_vec_Tensor (z#zs) y)!i) = (vec_vec_Tensor zs y)!0" using vec_vec_Tensor_simpl2
   by auto
  ...
  have " \<forall>i.((i < (length zs)*(length y))\<longrightarrow>((vec_vec_Tensor (zs) y)!i) 
    = f ((zs)!(i div (length y))) (y!(i mod (length y))))" using Cons.hyps by auto  
  from this 4 have 6:"(i = length y) \<longrightarrow>((0 < ((length zs)*(length y)))\<longrightarrow> (((vec_vec_Tensor (zs) y)!0) 
    = f ((zs)!0) (y!0))) = ((i < ((length zs)*(length y)))\<longrightarrow>(((vec_vec_Tensor zs y)!i) 
    = f ((zs)!(i div (length y))) (y!(i mod (length y)))))" by auto
  have 7: " (0 div (length y)) = 0" by auto
  have 8: " (0 mod (length y)) = 0" by auto
  have 9: "(0 < ((length zs)*(length y))) \<longrightarrow> ((vec_vec_Tensor zs y)!0) 
    = f (zs!0) (y!0)" using 7 8 Cons.hyps by auto
  from this 4 5 8 have "(i = length y) \<longrightarrow> ((i < (length (z#zs))*(length y)) \<longrightarrow> (((vec_vec_Tensor (zs) y)!0) 
    = f ((zs)!0) (y!0)))" 
  by auto
  from this 1 2 5 have "(i = length y) \<longrightarrow> ((i < (length (z#zs))*(length y)) \<longrightarrow> (((vec_vec_Tensor ((z#zs)) y)!i) 
    = f ((z#zs)!(i div (length y))) (y!(i mod (length y)))))" by auto
  from this show ?thesis by auto
  qed
 from this step2 have step4: " (i \<ge> (length y)) \<longrightarrow>  ((i < (length (z#zs))*(length y)) \<longrightarrow> (((vec_vec_Tensor ((z#zs)) y)!i) 
   = f ((z#zs)!(i div (length y))) (y!(i mod (length y)))))" by auto
 have "(i < (length y)) \<or> (i \<ge> (length y))" by auto
 from this step1 step4 have " ((i < (length (z#zs))*(length y)) \<longrightarrow> (((vec_vec_Tensor ((z#zs)) y)!i) 
   = f ((z#zs)!(i div (length y))) (y!(i mod (length y)))))" using logic by (metis "6" "7" 
  `i < length y \<longrightarrow> vec_vec_Tensor (z # zs) y ! i = z * y ! i`) 
 from this show ?case by auto
 qed
(*vec_mat_Tensor elements*)
>>>>>>> 7fd971d3a18e8b15f3ef550f3c1fd8e0c2e454ad
...
=======
 ...
 (length (a#M)) (a#M))
\<longrightarrow> ((vec_mat_Tensor v (a#M))!j!i) = f (v!(i div (row_length (a#M)))) ((a#M)!j!(i mod (row_length (a#M)))))"
  proof(cases a)
  case Nil
  have "row_length ([]#M) = 0" 
          using row_length_def by auto
  ...
  \<longrightarrow> ((vec_mat_Tensor v ([]#M))!j!i) = f (v!(i div (row_length ([]#M)))) ([]!j!(i mod (row_length ([]#M)))))"
  ...
  from this have 4:" \<forall>i.((i < (length a)*(length v)) \<longrightarrow>  
    ((vec_vec_Tensor v a)!i) = f (v!(i div (length a))) (a!(i mod (length a))))" using vec_vec_Tensor_elements Cons.hyps
  using nat_mult_commute by auto
  have "(vec_mat_Tensor v (a#M))!0 = (vec_vec_Tensor v a)" using vec_mat_Tensor.simps(2) by auto
  from this 2 4 have 5: " \<forall>i.((i < (row_length (a#M))*(length v)) \<longrightarrow>  
    ((vec_mat_Tensor v (a#M))!0!i) = f (v!(i div (row_length (a#M)))) ((a#M)!0!(i mod (row_length (a#M)))))" 
  by auto 
  ...
  from this 5 have 6: "(j = 0)\<longrightarrow>
   ...
   ((vec_mat_Tensor v (a#M))!j!i) = f (v!(i div (row_length (a#M)))) ((a#M)!j!(i mod (row_length (a#M))))))" 
   by auto 
  ...
  ((vec_mat_Tensor v (a#M))!j!i) = f (v!(i div (row_length (a#M)))) ((a#M)!j!(i mod (row_length (a#M)))))" 
   ...
   ((vec_mat_Tensor v (a#[]))!j!i) = f (v!(i div (row_length (a#[])))) ((a#[])!j!(i mod (row_length (a#[]))))))" 
   using 6 Nil by auto
