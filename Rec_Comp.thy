...
where
"cap_act n xs \<equiv>
(codom n, codom (n+1))#(split_codom_double_left_shift (\<lambda>x.(type x=codomain)\<and>(str_number x \<ge> n)) xs)"
...
|"prelim_other_end_list a (x#xs) =
           (if ((prelim_other_end_list a xs) \<noteq> a) then (prelim_other_end_list a xs) else a)"
...
"cup_act n xs \<equiv>
 (if 
    (belongs_to (codom n, codom (n+1)) xs)
  then
    split_codom_double_right_shift
      (\<lambda>x.(type x=codomain)\<and>(str_number x > (n+1))) (delete_from_list  (codom n, codom (n+1)) xs)
   else cup_check n xs)"
...
|"block_action (x#xs) ls =
   (case x of
     vert \<Rightarrow> ((vert_act (nat (domain_block xs)+1) (fst (block_action xs  ls))),(snd ls))
    |over \<Rightarrow> ((swap_act (nat (domain_block xs)+1)  (fst (block_action xs  ls)),(snd ls)))
|under \<Rightarrow> ((swap_act (nat (domain_block xs)+1)  (fst (block_action xs ls))),(snd ls))
|cap \<Rightarrow>    ((cap_act (nat (domain_block xs)+1)  (fst (block_action xs  ls))),(snd ls))
|cup \<Rightarrow>  ((cup_act (nat (domain_block xs)+1)  (fst (block_action xs  ls))),
 (if  (belongs_to (codom (nat (domain_block xs)+1), codom ((nat (domain_block xs))+2))  (fst (block_action xs  ls)))
...
(*problem lies with split_codomain_right*)
value "cup_check 1 []"
value "block_action [cup,cap,cup] ([],0)"
value "wall_action ([cap,cap]*(basic [cap,cap])) ([],0)"
